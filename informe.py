...
def main(authenticated=False):    
	...
	num_sucursales = df_filtrado[df_filtrado['Ingresos_Act'] > 0][df_filtrado['sucursal'] != 'Total']['sucursal'].nunique()    
        ...
        with col7:
            ui.metric_card(title="N° SUCURSALES", content=num_sucursales,  key="card7")            
        with col8:
        ...
        periodos = generar_periodos(numero_mes)         
        # Partiendo del dataframe df_filtrado_display    
        df_grouped = df_general.groupby('periodo')[['Ingresos_Act','Ingresos_Ant','Presupuesto']].sum().reset_index()
